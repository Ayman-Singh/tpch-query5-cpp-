                  ...
                  vector<map<string, string>>& region_data)
{
    ...
    // lineitem table
    {
        ...
        while (getline(in, line)) {
            ...
            // [0]=l_orderkey, [2]=l_suppkey, [5]=l_extendedprice, [6]=l_discount
            if (tokens.size() >= 7) {
                map<string, string> row;
...
bool executeQuery5(const string& r_name, const string& start_date, const string& end_date, int num_threads, const vector<map<string, string>>& customer_data, const vector<map<string, string>>& orders_data, const vector<map<string,string>>& lineitem_data, const vector<map<string, string>>& supplier_data, const vector<map<string,string>>& nation_data, const vector<map<string,string>>& region_data, map<string, double>& results) {
    ...
    // 2) Build a set of all nationkeys in that region, and a map nationKey → nationName
    unordered_set<int> nationsInAsia;
    unordered_map<int, string> nationNameByNationKey;
    for (auto &row : nation_data) {
        int nkey      = stoi(row.at("n_nationkey"));
        int regionkey = stoi(row.at("n_regionkey"));
        if (regionkey == asiaRegionKey) {
            nationsInAsia.insert(nkey);
            nationNameByNationKey[nkey] = row.at("n_name");
        }
    }
    if (nationsInAsia.empty()) {
        cerr << "Error: No nations found for region '" << r_name << "'.\n";
        return false;
    }

    // 3) Filter suppliers: s_suppkey → s_nationkey, but only if s_nationkey ∈ nationsInAsia
    ...
    // 4) Filter customers: c_custkey → c_nationkey, but only if c_nationkey ∈ nationsInAsia
    ...
    // 5) Filter orders: o_orderkey → o_custkey, if o_custkey ∈ customerNation AND date ∈ [start_date, end_date)
    ...
    if (orderCustKey.empty()) {
        cerr << "Error: No orders found for Asia customers in the given date range.\n";
        return false;
    }

    // 6) Scan lineitem_data in parallel and accumulate revenue per nation
    ...
    auto worker = [&](int startIdx, int endIdx) {
        for (int i = startIdx; i < endIdx; ++i) {
            ...
            // Only proceed if this lineitem’s order and supplier qualified
            ...
            string nname = nationNameByNationKey[suppNationKey];

            // Lock and add to sharedRevenue
            {
                lock_guard<mutex> lock(mtx);
                sharedRevenue[nname] += rev;
            }
        }
    };
    ...
    // 7) Copy sharedRevenue into results (map<string,double>)
